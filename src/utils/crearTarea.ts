import fp from "lodash";
import { Tarea } from "../models/Tarea";


export function crearTarea() {

    let nuevaTarea: Tarea;
    let datosValidos = false;

    while(!datosValidos)
    {
        let titulo = prompt("Título: ");
        let descripcion = prompt("Descripción: ") || "Sin descripcion";
        let estado = prompt("Estado [En curso / Pendiente / Terminada / Cancelada]: ") || "Pendiente";
        let dificultad = prompt("Dificultad [1] Fácil  [2] Medio [3] Difícil: ") || "1";
        let vencimiento = prompt("Vencimiento (DD-MM-AAAA): ") || "Sin datos";

        try  //cuando no se pone titulo no se crea la tarea y vuelve a pedir los datos
        {
        nuevaTarea = new Tarea(titulo, descripcion);
        datosValidos = true;
        }
        catch (error: unknown)
        {
            console.log("Error en la creacion de la tarea: " error.message);
            
        }
    }
    return nuevaTarea;
}